...
Matrix4x4 parse_transformation(const json &node) {
    ...
    for (auto it = transform_it->begin(); it != transform_it->end(); it++) {
        if (auto scale_it = it->find("scale"); scale_it != it->end()) {
            ...
            // construct a scale matrix and composite with F
	    ...
	    scaleMatrix(2, 2) = scale.z;
            ...
            // construct a rotation matrix and composite with F
 	    // axis components
	    Real x = axis.x;
	    Real y = axis.y;
	    Real z = axis.z;
	    ...
	    // first column
            rotateMatrix(0, 0) = (x * x) + ((1 - (x * x)) * cos(angle));
            rotateMatrix(0, 1) = ((x * y) * (1 - cos(angle))) + (z * sin(angle));
            rotateMatrix(0, 2) = ((x * z) * (1 - cos(angle))) - (y * sin(angle));
            rotateMatrix(0, 3) = 0;
	    // second column
	    rotateMatrix(1, 0) = ((y * x) * (1 - cos(angle))) - (z * sin(angle));
            rotateMatrix(1, 1) = (y * y) + ((1 - (y * y)) * cos(angle));
            rotateMatrix(1, 2) = ((y * z) * (1 - cos(angle))) + (x * sin(angle));
            rotateMatrix(1, 3) = 0;
	    // third column
	    rotateMatrix(2, 0) = ((z * x) * (1 - cos(angle))) + (y * sin(angle));
            rotateMatrix(2, 1) = ((z * y) * (1 - cos(angle))) - (x * sin(angle));
            rotateMatrix(2, 2) = (z * z) + ((1 - (z * z)) * cos(angle));
            rotateMatrix(2, 3) = 0;
            F = rotateMatrix * F;           
            ...
            // construct a translation matrix and composite with F
	    ...
	    lookatMatrix(0, 1) = r.y;
	    lookatMatrix(0, 2) = r.z;
	    lookatMatrix(0, 3) = 0;
	    // second column
	    lookatMatrix(1, 0) = u.x;
	    lookatMatrix(1, 1) = u.y;
	    lookatMatrix(1, 2) = u.z;
	    lookatMatrix(1, 3) = 0;
	    // third column
	    lookatMatrix(2, 0) = -d.x;
	    lookatMatrix(2, 1) = -d.y;
	    lookatMatrix(2, 2) = -d.z;
	    lookatMatrix(2, 3) = 0;
	    // third column
